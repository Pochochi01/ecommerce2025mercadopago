...
export const createNewPayment = createAsyncThunk('/payment/createNewPayment', async(orderData)=>{
    const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/shop/payment/create`, orderData);
    return response.data
})

export const capturePayment = createAsyncThunk('/payment/capturePayment', async({paymentId, payerId, orderId})=>{
    const response = await axios.post(`${import.meta.env.VITE_API_URL}/api/shop/payment/capture`, {paymentId, payerId, orderId});
    return response.data
})

export const getAllOrdersByUserId = createAsyncThunk('/payment/getAllOrdersByUserId', async(userId)=>{
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/shop/payment/list/${userId}`);
    return response.data
})

export const getOrderDetails = createAsyncThunk('/payment/getOrderDetails', async(id)=>{
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/shop/payment/details/${id}`);
    return response.data
})
