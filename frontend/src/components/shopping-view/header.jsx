  ...
  return (
    <div className="flex lg:items-center lg:flex-row flex-col gap-4">
      ...
      >
        ...
        <span className="sr-only">Tus Compras</span>
      </Button>
      <UserCartWrapper 
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side='right' className='w-56'>
          ...
          <UserCog className="mr-2 h-4 w-4" />Mi Perfil
          </DropdownMenuItem>
          <DropdownMenuSeparator/>
    ...
    <header className="sticky top-0 z-40 w-full border-b bg-background">
      <div className="flex h-16 items-center justify-between px-4 md:px6">
        <Link to="/shop/home" className="flex items-center gap-2">
          <img src={logoMenu} className="h-10 w-10 rounded-md" />
          {/*<House className="h-6 w-6" /> */}
          {/* Logo del negocio que redireccione a la pagina principal*/}
          <span className="font-bold">JatSport</span>
        </Link>
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" size="icon" className="lg:hidden">
              <Menu className="h-6 w-6" />
              <span className="sr-only">Alternar menu encabezado</span>
            </Button>
          </SheetTrigger>
